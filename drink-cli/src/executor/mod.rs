...
pub fn execute(app_state: &mut AppState) -> Result<()> {
	...
	let command = command.split_ascii_whitespace().map(|a| a.trim()).collect::<Vec<_>>();
	...
	match cli_command {
		...
		CliCommand::SetGasLimit { ref_time, proof_size } => {
		...
		CliCommand::Deploy { constructor, args, salt } =>
