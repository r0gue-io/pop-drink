...
/// let _address =
/// 	session.deploy(contract_bytes(), "new", NO_ARGS, NO_SALT, None, &get_transcoder())?;
	...
	T::Runtime: Config,
{
	...
	pub fn deploy_and<S: AsRef<str> + Debug>(
		...
		self.deploy(contract_bytes, constructor, args, salt, endowment, transcoder)
			.map(|_| self)
	}
	...
	/// argument.
	///
	/// You can get it with `ContractBundle::load("some/path/your.contract")` or
	/// `local_contract_file!()`
	...
	/// Similar to `deploy_and` but takes the parsed contract file (`ContractBundle`) as a first
	/// argument.
	///
	/// You can get it with `ContractBundle::load("some/path/your.contract")` or
	/// `local_contract_file!()`
	...
	///
	/// You can obtain it using `ContractBundle::load("some/path/your.contract")` or
	/// `local_contract_file!()`
	...
	pub fn call_and<S: AsRef<str> + Debug>(
		...
		// at decoding.
		self.call_internal::<_, ()>(None, message, args, endowment).map(|_| self)
	}
