...
pub fn build_contracts() -> BundleProviderGenerator {
	let metadata = MetadataCommand::new().exec().expect("Error invoking `cargo metadata`");
...
fn get_contract_crates(
	metadata: &Metadata,
) -> (Option<FeaturedPackage>, impl Iterator<Item = FeaturedPackage>) {
	let pkg_lookup = |id| {
		metadata
			.packages
			.iter()
	...
	let dep_graph = metadata.resolve.as_ref().expect("Error resolving dependencies");

	let contract_deps = dep_graph
		.nodes
		.iter()
		.filter(|node| node.features.contains(&INK_AS_DEPENDENCY_FEATURE.to_string()))
		.map(move |node| {
			let mut features_on = node.features.clone();
			features_on.retain(|feature| feature != INK_AS_DEPENDENCY_FEATURE && feature != "std");
			FeaturedPackage { package: pkg_lookup(node.id.clone()), features_on }
		});

	let root = dep_graph.root.as_ref().expect("Error resolving root package");
	let root = pkg_lookup(root.clone());

	(
		root.features
			.contains_key(INK_AS_DEPENDENCY_FEATURE)
			.then_some(FeaturedPackage { package: root, features_on: vec![] }),
		contract_deps,
	)
}
...
fn build_contract_crate(pkg: FeaturedPackage) -> (String, PathBuf) {
		...
		.entry(manifest_path.clone().into())
	{
		...
		Entry::Vacant(todo) => {
			...
			let bundle_path =
				result.metadata_result.expect("Metadata should have been generated").dest_bundle;
