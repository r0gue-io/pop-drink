use frame_support::{sp_runtime::DispatchError, traits::{fungibles::{Inspect, Create, Mutate}}};
        ...
        T::Runtime: pallet_assets::Config<Instance1>,
{
    ...
    fn balance_of(&mut self, asset: &AssetIdOf<T::Runtime>, owner: &AccountIdFor<T::Runtime>) -> BalanceOf<T::Runtime>;
...
impl<T> AssetsAPI<T> for T
    where
        ...
        T::Runtime: pallet_assets::Config<Instance1>,
{
    fn create(&mut self, id: &AssetIdOf<T::Runtime>, owner: &AccountIdFor<T::Runtime>, min_balance: BalanceOf<T::Runtime>) -> Result<(), DispatchError> {
    ...
    fn mint_into(
        ...
        self.execute_with(|| pallet_assets::Pallet::<T::Runtime, Instance1>::mint_into(asset.clone(), account, value))
    }

    fn balance_of(&mut self, asset: &AssetIdOf<T::Runtime>, owner: &AccountIdFor<T::Runtime>) -> BalanceOf<T::Runtime> {
        self.execute_with(|| pallet_assets::Pallet::<T::Runtime, Instance1>::balance(asset.clone(), owner))
    }

    fn asset_exists(&mut self, asset: &AssetIdOf<T::Runtime>) -> bool {
        self.execute_with(|| pallet_assets::Pallet::<T::Runtime, Instance1>::asset_exists(asset.clone()))
    }
}
...
mod test {
    ...
    fn api_works() {
        ...
        sandbox
            .create(&token, &DefaultSandbox::default_actor(), 1)
            .unwrap();
        sandbox
            .mint_into(&token, &DefaultSandbox::default_actor(), 100)
            .unwrap();

        assert_eq!(
            sandbox.balance_of(&token, &DefaultSandbox::default_actor()),
            balance + 100
        );
